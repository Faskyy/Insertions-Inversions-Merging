...
public class Exercise1Lab9 {
   ...
   public static void main(String[] args) {
      ...
      int count = merge(ab, aux, lo, mid, hi);

      System.out.print("Sorted array ab (merged a and b):    " + Arrays.toString(ab));
      // YOUR CODE HERE 
       
       System.out.println("The number of inversions in the concatenated array is ..." + count);
   }
   ...
   public static int merge(int[] a, int[] aux, int lo, int mid, int hi) {
      ...
      int i = lo, j = mid+1;
    	 for (int k = lo; k <= hi - 1; k++) {
    		 if      (i > mid)              {a[k] = aux[j++]; count++;}
    		 else if (j > hi)               a[k] = aux[i++];
         
         //this line means inversion because aux[j] < [i]
